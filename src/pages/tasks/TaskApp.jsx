...
import { useNavigate } from 'react-router-dom';
    ...
    import CaiTienTab from './CaiTienTab';
    ...
    function App() {
      ...
      const tabItems = [
        ...
        canSeeTab(me,'tasks','cai_tien') && h(TabButton, {key:'cai_tien', active: view==='cai_tien', onClick: ()=>navTo('cai_tien'), label:'Cải tiến', color:'#2563eb'}),
      ].filter(Boolean)
        ...
        h('main',{style:{flex:1,width:'100%',overflowX:'hidden',padding:view==='menu'?'1rem 1rem 4rem':'0.75rem 0.75rem 4rem',background:'var(--bg-primary)',minHeight:'calc(100vh - 90px)', display:'flex', flexDirection:'column'}},
          busy ? h('div',{className:'flex items-center justify-center h-64'}, h('div',{className:'text-blue-500 font-bold animate-pulse text-sm'},'Đang tải...'))
               : view==='menu' ? h('div', {
                   ...
                   canSeeTab(me,'tasks','kpi') && h('button', {
                     onClick: () => navTo('kpi'),
                     style: { background: '#fff', borderRadius: 16, border: '1px solid #e2e8f0', padding: '1.25rem 0.5rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', transition: 'all 0.25s', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.07)' }
                   },
                   ...
                   canSeeTab(me,'tasks','cai_tien') && h('button', {
                     onClick: () => navTo('cai_tien'),
                     style: { background: '#fff', borderRadius: 16, border: '1px solid #e2e8f0', padding: '1.25rem 0.5rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', transition: 'all 0.25s', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.07)' }
                   },
                     h('div', {style: {width: 46, height: 46, borderRadius: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #2563eb, #3b82f6)', boxShadow: '0 4px 12px rgba(37, 99, 235, 0.2)'}}, h(Lightbulb, {size: 24, color: '#fff'})),
                     h('h3', {style: {margin: 0, fontSize: '0.8rem', fontWeight: 700, color: '#0f172a', textAlign: 'center'}}, 'Cải tiến')
                   )
                 )
