...
const AdminDashboard = () => {
  ...
  const fetchAllData = useCallback(async (forceRefresh = false) => {
    ...
    try {
      const [
        ...
        fetchAllRows(() => supabase.from('cong_viec_duoc_giao').select('id, title, assignee_id, status, completed_date, due_date')),
      ]);
  ...
  useEffect(() => {
    ...
    tLogs.forEach(task => { let isDoneInRange = false; if (task.status === 'COMPLETED' && task.completed_date) { const cd = task.completed_date.split('T')[0]; if (cd >= startStr && cd <= endStr) isDoneInRange = true; } const isPending = task.status !== 'COMPLETED' && task.status !== 'CANCELLED'; if (!isDoneInRange && !isPending) return; const wId = task.assignee_id; cTasksTotal += 1; if (isDoneInRange) cTasksDone += 1; activeStaff.add(wId); if (wId && staffMap.has(wId)) { const st = staffMap.get(wId); st.tasksTotal += 1; if (isDoneInRange) { st.tasksDone += 1; st.tasksDoneList.push(task.title); let isLate = false; if (task.due_date && task.completed_date) { const due = new Date(task.due_date).getTime(); const done = new Date(task.completed_date).getTime(); if ((done - due) > 60000) isLate = true; } if (!isLate) { st.tasksOnTime += 1; cTasksOnTime += 1; } } } });
