-- ==============================================================================
-- BẢN CẬP NHẬT 11: FIX LỖI ĐẢO VAI CÂU HỎI/TRẢ LỜI & WEBHOOK RACE CONDITION
-- ==============================================================================
...
BEGIN
    ...
    -- Lấy tên nhóm từ n8n (nếu có truyền)
    ...
    -- Kiểm tra xem người gửi có phải là nhân viên không (Dựa vào bảng nhan_vien)
    -- Đảm bảo is_staff LUÔN được xác định bởi bảng nhan_vien, không bao giờ dựa vào trạng thái hội thoại.
    SELECT EXISTS(
    ...
    IF NEW.is_staff = true THEN
        ...
        IF v_customer_uid IS NOT NULL THEN
            ...
            IF v_conv_id IS NOT NULL THEN
                -- Gán trả lời vào hội thoại đang mở
            ...
            ELSE
                -- NHÂN VIÊN NHẮN TRƯỚC / WEBHOOK NV VỀ TRƯỚC
                -- Tạo một hội thoại "Đã Xử Lý" (Buffer)
            ...
            END IF;
        ELSE
            -- Nội bộ: Không tạo hội thoại
            v_conversation_id := NULL;
        END IF;

    ELSE
        -- NHÁNH KHÁCH HÀNG NHẮN (Tạo Yêu Cầu Chờ Xử Lý)
        ...
        WHERE thread_id = NEW.thread_id
          ...
          AND (
              ...
              OR 
              -- Ghép ngược nếu NV nhắn trước (Buffer delay)
        ...
        IF v_conv_id IS NOT NULL THEN
            IF v_is_responded = true AND v_message_count = 0 THEN
                -- GHÉP NGƯỢC (Khách nhắn sau khi NV đã tạo Buffer)
                UPDATE public.zalo_conversations
                SET first_message_ts = NEW.ts, -- Giờ mới có câu hỏi đầu tiên
                    last_message_ts = NEW.ts,
                    message_count = 1,
                    response_time_ms = response_ts - NEW.ts, -- Sửa lại tgian phản hồi thực tế (dù có thể âm nếu webhook NV đến cực nhanh, nhưng ko sao)
                ...
                RETURNING id INTO v_conversation_id;
            ELSE
                -- GOM BÌNH THƯỜNG (Khách nhắn tiếp trong 10 phút)
                UPDATE public.zalo_conversations
