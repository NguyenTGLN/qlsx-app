-- ==============================================================================
-- BẢN CẬP NHẬT 13: TRA CỨU QUOTE BẰNG globalMsgId + NGƯỠNG GOM 10 PHÚT
-- ==============================================================================
...
DECLARE
    ...
    v_group_name TEXT;
    v_customer_uid TEXT;
    v_quote_global_msg_id TEXT := NULL;
BEGIN
    ...
    -- Lấy UID người bị quote/mention
    ...
    -- Lấy globalMsgId của tin nhắn gốc bị quote
    v_quote_global_msg_id := COALESCE(
        NEW.raw_data->'data'->'quote'->>'globalMsgId',
        NEW.raw_data->'data'->'quote'->>'cliMsgId'
    );
    ...
    IF NEW.is_staff = true THEN
        ...
        IF v_customer_uid IS NOT NULL THEN
            -- Bước 2a: Có Quote → Tra globalMsgId để tìm CHÍNH XÁC hội thoại
            IF v_quote_global_msg_id IS NOT NULL THEN
                SELECT conversation_id INTO v_conv_id
                FROM public.zalo_messages
                WHERE (raw_data->'data'->>'msgId' = v_quote_global_msg_id
                    OR raw_data->'data'->>'globalMsgId' = v_quote_global_msg_id
                    OR raw_data->'data'->>'cliMsgId' = v_quote_global_msg_id)
                  AND conversation_id IS NOT NULL
                LIMIT 1;

                IF v_conv_id IS NOT NULL THEN
                    SELECT first_message_ts INTO v_conv_ts
                    FROM public.zalo_conversations WHERE id = v_conv_id;
                END IF;
            END IF;

            -- Bước 2b: Không tìm thấy bằng msgId → Fallback: tìm hội thoại chưa trả lời gần nhất
            ...
            -- Bước 2c: Fallback cuối → tìm bất kỳ hội thoại gần nhất (kể cả đã trả lời)
            IF v_conv_id IS NULL THEN
                SELECT id, first_message_ts INTO v_conv_id, v_conv_ts
                FROM public.zalo_conversations
                WHERE is_group = v_is_group
            ...
            IF v_conv_id IS NOT NULL THEN
                -- Gán trả lời vào hội thoại tìm được
            ...
            END IF;
        ELSE
            -- Nhóm nhưng không Quote ai → Nội bộ
    ...
    ELSE
        ...
        WHERE is_group = v_is_group
          ...
          AND customer_uid = v_customer_uid
          AND (
              -- Gom bình thường: cùng khách, chưa trả lời, trong 10 phút
              (is_responded = false AND (NEW.ts - last_message_ts) <= 600000)
              OR 
              -- Ghép ngược Buffer: NV nhắn trước, trong 5 phút
