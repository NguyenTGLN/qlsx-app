-- ==============================================================================
-- BẢN CẬP NHẬT 10: SỬA LỖI NHÂN VIÊN BỊ NHẬN DIỆN LÀ KHÁCH HÀNG (DO KHÔNG QUOTE)
-- ==============================================================================
...
BEGIN
    ...
    IF v_conv_id IS NOT NULL THEN
        -- Đã tìm thấy hội thoại của khách hàng mà người này đang trả lời (Quote)
        NEW.is_staff := true;
    ...
    ELSE
        -- Tình huống không Quote ai cả
        IF NEW.is_staff = true THEN
            -- Là nhân viên nhưng không Quote khách -> Chat nội bộ -> Không tạo Ticket KPI mới
            v_conversation_id := NULL;
        ELSE
            -- Là khách hàng -> Tìm phiên chat gần nhất để gộp, hoặc tạo mới
