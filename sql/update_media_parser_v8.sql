-- ==============================================================================
-- BẢN CẬP NHẬT 8: TỰ ĐỘNG TRA CỨU TÊN NHÓM TỪ BẢNG ZALO_GROUPS
-- ==============================================================================
...
BEGIN
    ...
    -- 🔴 MỚI: NẾU LÀ TIN NHẮN NHÓM MÀ CHƯA CÓ TÊN, THÌ TRA CỨU TRONG BẢNG zalo_groups
    IF v_is_group AND (v_group_name IS NULL OR v_group_name = '') THEN
        BEGIN
            -- Giả định bảng zalo_groups có cột "id" và "name" (hoặc "ten_nhom")
            -- Dùng khối EXCEPTION để tránh lỗi sập Trigger nếu sai tên cột
            EXECUTE 'SELECT name FROM public.zalo_groups WHERE id::text = $1 LIMIT 1' 
            INTO v_group_name 
            USING NEW.thread_id;
        EXCEPTION WHEN others THEN
            -- Nếu lỗi (vd: không có cột name), thử tìm cột ten_nhom
            BEGIN
                EXECUTE 'SELECT ten_nhom FROM public.zalo_groups WHERE id::text = $1 LIMIT 1' 
                INTO v_group_name 
                USING NEW.thread_id;
            EXCEPTION WHEN others THEN
                -- Bỏ qua nếu vẫn lỗi
