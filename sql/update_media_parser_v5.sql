-- ==============================================================================
-- BẢN CẬP NHẬT 5: SỬA LỖI KHÁCH HÀNG / NHÂN VIÊN TRẢ LỜI LẠI CÂU TRẢ LỜI CỦA NHÂN VIÊN
-- ==============================================================================
...
BEGIN
    ...
    -- Kiểm tra xem người bị Quote có phải là Nhân viên (người đã từng trả lời trong group này) không?
    ...
    -- Bỏ qua nếu Quote chính mình, hoặc Quote một Nhân viên (v_is_target_a_responder = true)
    IF v_target_uid IS NOT NULL AND v_target_uid != NEW.uid_from AND v_is_target_a_responder = false THEN
        ...
        WHERE thread_id = NEW.thread_id
          ...
          AND (NEW.ts - last_message_ts) <= 259200000 -- Chỉ nối vào tin nhắn trong vòng 3 ngày
        ORDER BY last_message_ts DESC
        LIMIT 1;
    END IF;

    -- NẾU TÌM THẤY HỘI THOẠI -> ĐÂY LÀ NHÂN VIÊN TRẢ LỜI KHÁCH HÀNG (Tự đối chiếu)
    ...
    -- NẾU KHÔNG CÓ QUOTE, HOẶC QUOTE TRÚNG NHÂN VIÊN -> LÀ TIN NHẮN TẠO YÊU CẦU MỚI
    ELSE
        NEW.is_staff := false;
