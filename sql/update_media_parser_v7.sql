-- ==============================================================================
-- BẢN CẬP NHẬT 7: BỔ SUNG TÊN NHÓM VÀO BẢNG ZALO CONVERSATIONS
-- ==============================================================================

-- 1. Thêm cột group_name vào bảng (Nếu chưa có)
ALTER TABLE public.zalo_conversations ADD COLUMN IF NOT EXISTS group_name TEXT;

-- 2. Cập nhật lại Trigger để lấy Tên nhóm từ dữ liệu n8n truyền vào
...
BEGIN
    ...
    -- Lấy tên nhóm nếu có truyền vào từ n8n
    v_group_name := COALESCE(
        NEW.raw_data->'data'->>'groupName', 
        NEW.raw_data->>'groupName', 
        NEW.raw_data->'data'->>'source_name' -- Đôi khi source_name là tên nhóm
    );
