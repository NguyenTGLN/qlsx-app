...
### 4.2 AssigneeList — hiện ĐỦ mọi thành viên (cập nhật 2026-07-17)

**Quyết định đã đổi:** ban đầu định rút gọn `Ngọc +2` cho vừa cột avatar hẹp, nhưng như vậy giấu
tên đi — người xem phải mở chi tiết mới biết ai làm. Người dùng yêu cầu: **mọi chỗ hiển thị đều
phải liệt kê đủ thành viên.** Vì thế `AvatarGroup`/`AvatarName` bị thay bằng `AssigneeList`:

- Mỗi thành viên là một chip `avatar + tên`, xếp ngang, tự xuống dòng khi chật (`flex-wrap`).
- Mảng rỗng → `Chưa giao`.
- Không còn khái niệm `+N` ở bất kỳ đâu.

Layout phải nhường chỗ cho danh sách đầy đủ:
- Thẻ "Việc đang thực hiện" (Tổng quan): bỏ cột avatar hẹp bên trái, đưa người thực hiện thành
  một hàng riêng chiếm trọn bề ngang thẻ.
- Bảng việc: cột "Người" nới từ 15% lên 22% (bớt ở cột Cập nhật), ô bỏ `nowrap` để chip xuống dòng.
- Panel chi tiết việc (`TaskDetail`): liệt kê đủ như trước.
...
> **Đổi 2026-07-17:** `TvDashboard.jsx` ban đầu để ngoài phạm vi, nay đã đưa vào — TV tự tra tên
> từ `assignee_ids` và hiện đủ nhóm (giới hạn 2 dòng để không vỡ thẻ trên màn hình lớn).
